...
**这是一条学习路径，不是一个框架。** 你会从 Rust 环境直接起步，再研究三个真实的 agent harness——各司其职——并用 Rust 逐里程碑地重写其中最小、最干净的那个（[pi](https://github.com/badlogic/pi-mono)）。走完之后你同时拥有三样东西：一套关于 coding-agent harness 如何真正工作的认知模型、一个你亲手写出的真实 Rust 代码库、以及用"真正发布过的实现"去评判任何 agent 框架的判断力。
...
| 仓库 | 角色 | 它给你什么 |
|---|---|---|
| `rust-course/`（Rust，本仓库内） | **教科书** | 用小型可运行 Rust lab 承接核心 harness 概念：Cargo workspace、事件流、agent loop、工具分发、compaction。每个里程碑开工前先跑对应 lab，让直觉直接长在 Rust 里。 |
...
```
① 热身   —— 跑通 `rust-course` 对应 lab（Rust 可运行直觉，约半小时）
...
# 从 Rust 开始，不需要 API key：
cargo run -p rust-course --bin m0_hello_rust
cargo run -p rust-course --bin m3_agent_loop
```

你的正式 Rust 重写放在 `pi-rs/`——M0 时自己 `cargo init`，这本身就是练习的一部分。`rust-course/` 只是热身实验台。
...
2. 做[阅读地图](docs/zh/01-reading-map.md)的"第零步"：把 Rust 裸 loop 跑在手上（约半天）。
...
- [learn-claude-code](https://github.com/shareAI-lab/learn-claude-code)（shareAI-lab）——渐进式教学结构的灵感来源；本仓库现在把起步路径改写成 Rust lab。
